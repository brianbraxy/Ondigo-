    ...
    <div class="box box-default">
        <div class="box-body">
            <div class="d-flex justify-content-between">
                <div>
                    <div class="top-bar-title padding-bottom pull-left">{{ __('Cards') }}</div>
                </div>

                <div>
                    @if(Common::has_permission(auth('admin')->user()->id, 'add_card'))
                        <a href="{{url(config('adminPrefix').'/cards/create')}}" class="btn btn-theme f-14"><span class="fa fa-plus"> &nbsp;</span>{{ __('Add Card') }}</a>
                    @endif
                </div>
            </div>
        </div>
