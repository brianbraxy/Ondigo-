<!-- Footer section -->
<div
    class="footer-sec {{ request()->is('login') || request()->is('login/agent') || request()->is('register/agent') || request()->is('forget-password') || request()->is('register') || request()->is('register/store-personal-info') || request()->is('2fa') || request()->is('google2fa') ? 'd-none' : '' }}">
    <div class="px-240">
        <div class="row">
            <div class="col-md-5 pb-54 align-center">
                <a class="d-flex" href="{{ request()->path() != 'merchant/payment' ? url('/') : 'javascript:void(0)' }}">
                    <img class="mt-54 footer-logo" src="{{ asset('uploads/logos/ondigo.svg') }}"
                        alt="{{ __('Brand Logo') }}">
                </a>

                <p class="pb-0 mt-26 w-358 txt-center text-white gilroy-light leading-29">
                    ...
                    @foreach (getAppStoreLinkFrontEnd() as $app)
                        @if (!empty($app->logo))
                            <a href="{{ $app->link }}" target="_blank">
                                <img class="{{ $app->company == 'Apple' ? 'ms-3 ml-r12' : '' }}app-imgs"
                                    src="{{ url('public/uploads/app-store-logos/thumb/' . $app->logo) }}"
                                    alt="{{ __('playstore') }}">
                            </a>
                        @else
                            <a href="javascript:void(0)">
                                <img class="app-imgs"
                                    src="{{ url('public/uploads/app-store-logos/default-logo.jpg') }}">
                            </a>
                        @endif
                    @endforeach
                    ...
                    <ul class="links gilroy-light">
                        ...
                        @if (!empty(getMenuContent('Footer')))
                            @foreach (getMenuContent('Footer') as $footer_navbar)
                                <li>
                                    <a href="{{ url($footer_navbar->url) }}"
                                        class="poppins5">{{ $footer_navbar->name }}</a>
                                </li>
                            @endforeach
                        @endif
...
<div
    class="{{ request()->is('login') || request()->is('login/agent') || request()->is('register/agent') || request()->is('forget-password') || request()->is('register') || request()->is('register/store-personal-info') || request()->is('2fa') || request()->is('google2fa') ? 'd-none' : '' }} bottom-footer">
        ...
        <div>
            <div class="d-flex align-center justify-center-res sp mt-18">
                ...
                <div class="form-group OpenSans-600 selectParent footer-font-16 OpenSans-600">
                    <select class="select2 form-control footer-font-16 mb-2n" data-minimum-results-for-search="Infinity"
                        id="lang">
                        @foreach (getLanguagesListAtFooterFrontEnd() as $lang)
                            <option class="footer-font-16"
                                {{ \Session::get('dflt_lang') == $lang->short_name ? 'selected' : '' }}
