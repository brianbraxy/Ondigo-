<!-- navbar -->
<div
    class="navigation-wrap bg-white start-header start-style {{ request()->is('login') || request()->is('login/agent') || request()->is('register/agent') || request()->is('forget-password') || request()->is('password/resets/*') || request()->is('register') || request()->is('register/store-personal-info') || request()->is('2fa') || request()->is('google2fa') ? 'd-none' : '' }}">
    <div class="container-fluid px-240">
        <div class="row">
            <div class="col-12">
                <nav class="navbar navbar-expand-md navbar-light">
                    <a class="navbar-brand"
                        href="{{ request()->path() != 'merchant/payment' ? url('/') : 'javascript:void(0)' }}">
                        ...
                        <ul class="navbar-nav ms-auto py-4 py-md-0 gilroy-medium nav-align">
                            ...
                            @if (!empty(getMenuContent('Header')))
                                @foreach (getMenuContent('Header') as $top_navbar)
                                    <li
                                        class="nav-item {{ isset($menu) && $menu == $top_navbar->url ? 'nav-active' : '' }}">
                                        <a href="{{ url($top_navbar->url) }}" class="nav-link">
                                            {{ $top_navbar->name }}</a>
                                    </li>
                                @endforeach
                            @endif
                            ...
                            <li class="nav-item {{ isset($menu) && $menu == 'privacy-policy' ? 'nav-active' : '' }}">
                                <a class="nav-link"
                                    href="{{ route('privacy_policy') }}">{{ __('Privacy Policy') }}</a>
                            </li>
                            ...
                            <li>
                                <div class="color-parent mt-2p">
                                    <div class="switch">
                                        <div id="switch">
                                            <img src="{{ asset('/frontend/templates/images/home/moon.png') }}"
                                                class="moon img-none" width="26px" alt="">
                                            <img src="{{ asset('/frontend/templates/images/home/sun2.png') }}"
